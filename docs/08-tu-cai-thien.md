...
Trang "Tự cải thiện" cho phép Boss tự thức theo lịch, làm **một nhiệm vụ cụ thể** rồi tự kiểm chứng và ghi nhật ký, mà không cần bạn ngồi gõ lệnh. Đây là "vòng lặp tự cải thiện" chạy nền ngay trên máy/VPS của bạn.
...
Hãy hình dung một trợ lý nền: cứ mỗi khoảng thời gian bạn đặt (ví dụ 60 phút), Boss tự mở vault ra, chọn đúng một việc theo mục tiêu bạn cấu hình, làm việc đó, rồi ghi lại kết quả vào nhật ký. Nếu bạn để chế độ "Tự làm", nó còn chạy thêm một bước tự kiểm chứng để soi xem việc vừa làm có đáng tin không.

Bốn kiểu nhiệm vụ có sẵn:

| Loại nhiệm vụ | Mã | Boss làm gì |
|---|---|---|
...
| Cải thiện Boss | `product` | Đọc log hội thoại gần đây rồi đề xuất hoặc tạo agent, workflow, hoặc ghi note cải tiến trải nghiệm. |
| Tự định nghĩa | `custom` | Bạn tự mô tả nhiệm vụ cụ thể bằng lời của mình (xem phần bên dưới). |

Trang này khác với trang [Lịch & tự động hoá](12-lich-tu-dong-hoa.md): trang đó quản lý các cron/trigger/routine chạy trên cloud của Claude. Trang "Tự cải thiện" là **vòng lặp nội bộ** chạy ngay trong server Boss của bạn. Thực tế, vòng lặp này cũng hiện ra ở trang Lịch dưới tên "Vòng lặp tự cải thiện" (một routine gắn cứng, chỉ bật/tắt được chứ không xoá).

## Mở ở đâu trong Boss

1. Mở dashboard Boss (mặc định ở cổng 7777).
...
Ở mục **Loại nhiệm vụ**, bấm một trong bốn nút: **Kinh doanh**, **Bộ não (Wiki)**, **Cải thiện Boss**, hoặc **Tự định nghĩa**. Nút được chọn sẽ sáng màu cam. Ngay dưới đó có một dòng mô tả ngắn nhắc bạn loại nhiệm vụ đó làm gì.
...
Nếu bạn mới dùng, hãy bắt đầu bằng **Đề xuất (ghi nháp)** cho an tâm, xem Boss đề xuất gì trước, rồi mới chuyển sang **Tự làm + kiểm chứng** khi đã tin tưởng.

Ở chế độ "Tự làm", Boss chỉ được dùng bộ công cụ file (đọc, viết, sửa). Ở chế độ "Đề xuất", nó còn bị siết chặt hơn: chỉ được đọc.
...
Lưu ý: bộ đếm lịch của Boss kiểm tra mỗi khoảng ngắn xem đã tới hạn chưa, nên thời điểm chạy thật có thể lệch vài chục giây so với con số bạn đặt. Điều này bình thường.
...
Đây là chế độ linh hoạt nhất. Khi chọn **Tự định nghĩa**, ô "Mô tả nhiệm vụ cụ thể" hiện ra. Bạn viết bằng lời thường, càng cụ thể càng tốt: nói rõ Boss đọc gì, làm gì, và lưu kết quả vào đâu.
...
Không muốn chờ tới chu kỳ? Bấm **▶ Chạy ngay**. Boss lưu cấu hình hiện tại rồi khởi động ngay một vòng. Nút đổi thành "Đang chạy..." trong giây lát. Sau đó xem ô trạng thái và "Nhật ký gần đây" để đọc kết quả.
...
Phần **Nhật ký gần đây** liệt kê các entry log mới nhất. Mỗi lần chạy, Boss ghi một mục gồm: loại nhiệm vụ, chế độ, lý do chạy (thủ công hay theo lịch), phần tóm tắt việc đã làm, và dòng kiểm chứng nếu có.
...
Khi bạn để **Tự làm + kiểm chứng** và Boss thật sự có làm việc gì đó, nó chạy thêm một lượt kiểm tra độc lập: một "người soi" giả định kết quả vừa rồi là SAI, rồi đọc lại file liên quan để đối chiếu. Tiêu chí kiểm tra tùy loại nhiệm vụ:
...
Kết quả kiểm chứng hiện dưới dạng **✓ Đạt** hoặc **✗ Chưa đạt** kèm lý do ngắn, cả trong ô trạng thái lẫn trong nhật ký. Đây là lưới an toàn thứ hai, giúp bạn không phải tin mù vào việc Boss tự làm.
...
Bấm nút, Boss quét thư mục Wiki và trả về một **danh sách kiểm tra** gồm 8 loại vấn đề thường gặp:
...
LINT **chỉ đọc và liệt kê, không tự sửa**. Bạn xem danh sách rồi tự quyết định muốn sửa gì. Nếu Wiki sạch, Boss sẽ nói rõ. Muốn hiểu sâu hơn về Wiki và bộ nhớ, xem [Second Brain: bộ nhớ, Wiki, INGEST](13-second-brain-bo-nho-wiki.md).

## An toàn: những gì Boss KHÔNG làm ở đây
...
- **KHÔNG** sửa code của server Boss.

Nói cách khác, tệ nhất thì nó tạo ra một note nháp không hay; nó không thể làm điều gì gây tốn tiền hay ảnh hưởng ra bên ngoài. Nếu bạn muốn Boss thật sự thực thi hành động ra ngoài (gửi tin, tạo đơn...), việc đó nằm ở luồng trò chuyện có bạn giám sát, không phải ở vòng lặp nền này.

## Mẹo

1. **Bắt đầu bằng chế độ Đề xuất.** Cho Boss chạy vài vòng ở "Đề xuất (ghi nháp)" để xem chất lượng đề xuất, rồi mới bật "Tự làm + kiểm chứng".
...
6. **Đọc nhật ký sau vài ngày.** Sau khi bật, thỉnh thoảng ghé lại xem "Nhật ký gần đây" để biết Boss đang làm gì và điều chỉnh mục tiêu nếu cần.
...
**Kết quả báo "Claude CLI chưa cài".** Bộ não Boss (Claude Code CLI) chưa sẵn sàng. Xem [Bắt đầu & thiết lập lần đầu](01-bat-dau-thiet-lap.md) và [Khắc phục sự cố & FAQ](17-khac-phuc-su-co.md).

**Kiểm chứng báo "✗ Chưa đạt".** Nghĩa là bước tự soi thấy việc Boss vừa làm chưa ổn (ví dụ bịa số, sai quy ước Wiki). Đọc lý do trong nhật ký, mở file liên quan qua [Quản lý tệp tin](05-quan-ly-tep-tin.md) để kiểm tra và sửa tay nếu cần.
...
- [Quản lý tệp tin](05-quan-ly-tep-tin.md) - mở file nhật ký và note nháp Boss tạo ra.
