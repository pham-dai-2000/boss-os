...
Đây là màn hình chính của Boss OS: quả cầu tri thức 3D ở giữa, khung hội thoại bên phải, số liệu kinh doanh bên trái. Bạn ra lệnh cho Boss OS bằng cách gõ chữ hoặc nói, và Boss OS trả lời bằng chữ kèm đọc ra tiếng.

Nếu chưa cài đặt xong lần đầu, xem [Bắt đầu & thiết lập lần đầu](01-bat-dau-thiet-lap.md) trước.

## Tính năng này là gì

Một chỗ duy nhất để làm việc với Boss OS:

- Gõ tin nhắn như chat bình thường.
- Nói bằng giọng, Boss OS nghe rồi tự gửi khi bạn ngừng nói.
- Boss OS trả lời bằng chữ, đồng thời đọc thành tiếng bằng giọng Việt.
- Đính kèm file hoặc ảnh vào tin nhắn để Boss OS đọc.
- Xem quả cầu tri thức phản ứng theo âm thanh (sáng lên khi nghe / khi đọc).

Câu trả lời được xử lý bởi bộ não Claude Code chạy nền. Trong lúc Boss OS suy nghĩ, một thanh trạng thái nhỏ hiện các bước Boss OS đang làm (gọi công cụ, đọc dữ liệu...).

## Mở ở đâu trong Boss OS

Màn hình này chính là mục **Boss OS (3D)** trên thanh điều hướng bên trái, cũng là màn hình mặc định khi bạn mở dashboard (mặc định ở cổng 7777). Không cần bấm gì thêm, mở trang lên là đã ở đây.
...
| HỘI THOẠI | Cột phải | Lịch sử chat với Boss OS |
...
1. Bấm vào ô nhập ở dưới cùng (chỗ ghi "Nói với Boss OS, gõ ở đây, hoặc kéo/dán file vào...").
2. Gõ câu hỏi.
...
Câu trả lời của Boss OS hiện dần ở cột HỘI THOẠI bên phải, chữ chạy ra theo thời gian thực.
...
3. Nói câu của bạn. Chữ bạn nói hiện ngay dưới trạng thái để bạn thấy Boss OS nghe đúng chưa.
4. **Thả phím Cách** ra. Boss OS tự gửi toàn bộ câu vừa nói và bắt đầu trả lời.

Lần đầu bấm mic, trình duyệt sẽ hỏi quyền dùng micro. Bấm cho phép. Nếu từ chối, Boss OS không nghe được và sẽ báo "Anh cần cấp quyền microphone cho trang này.".
...
2. Cứ nói tự nhiên. Khi bạn ngừng nói một chút (khoảng 1,5 giây im lặng), Boss OS tự chốt câu và gửi đi.
3. Sau khi trả lời xong, Boss OS tự bật mic nghe lại, không cần bạn bấm.
4. Muốn tắt chế độ này: bấm lại nút mic, hoặc nhấn phím **Esc**.

Trong chế độ rảnh tay, khi bạn bắt đầu nói thì Boss OS tự ngắt phần nó đang đọc để lắng nghe, nên bạn có thể chen ngang bất cứ lúc nào.

### Nghe Boss OS trả lời bằng giọng

Mặc định Boss OS **đọc thành tiếng** mọi câu trả lời bằng giọng Việt (công nghệ Edge TTS trên máy chủ). Quả cầu 3D sáng theo nhịp giọng đọc.

Bật/tắt đọc bằng giọng, có 2 chỗ làm cùng một việc:

- Nút hình **loa** ở góc trên phải (tên gợi ý khi rê chuột: "Bật/tắt giọng Boss OS"). Loa gạch chéo = đang tắt tiếng.
...
4. "Ngôn ngữ nghe" là ngôn ngữ Boss OS dùng để nhận diện lời bạn nói, khác với giọng đọc trả lời. Để mặc định Tiếng Việt trừ khi bạn quen nói tiếng Anh.

Mọi lựa chọn giọng, tốc độ, ngôn ngữ nghe đều được ghi nhớ cho lần sau.

### Gửi file kèm trong chat

Bạn có thể đưa file hoặc ảnh vào tin nhắn để Boss OS đọc. Ba cách:

1. Bấm nút **kẹp giấy** (bên cạnh nút mic) rồi chọn file. Có thể chọn nhiều file.
2. **Kéo - thả** file từ máy vào cửa sổ Boss OS (một lớp phủ hiện lên báo chỗ thả).
...
Quan trọng về cách Boss OS xử lý file:

- **Mặc định: chỉ đọc.** Boss OS đọc nội dung file (ảnh thì xem và mô tả) rồi trả lời, **không** tự lưu vào đâu.
- **Chỉ lưu khi bạn yêu cầu rõ.** Muốn Boss OS cất file vào bộ nhớ (Second Brain), hãy nói rõ trong tin nhắn, ví dụ "lưu vào source", "ingest cái này", hoặc "ghi vào second brain". Khi đó Boss OS mới chuyển file thành ghi chú và lưu vào thư mục Sources của vault. Xem thêm [Second Brain: bộ nhớ, Wiki, INGEST](13-second-brain-bo-nho-wiki.md) và [Quản lý tệp tin](05-quan-ly-tep-tin.md).

### Dừng khi Boss OS đang trả lời

Khi Boss OS đang suy nghĩ hoặc đang đọc, nút gửi ở thanh nhập biến thành **nút dừng** (hình vuông). Có 2 cách ngắt:
...
- Rê chuột vào câu trả lời của Boss OS sẽ thấy nút **⧉** để copy cả tin nhắn; mỗi khối code cũng có nút **⧉ Copy** riêng ở góc.
- Tin nhắn dài của bạn được thu gọn sau 10 dòng, bấm **Xem thêm** để mở.
- Khi bạn đang cuộn lên đọc lại mà Boss OS trả lời tiếp, khung chat KHÔNG giật xuống; một nút **↓ Tin mới** hiện ở đáy để bấm nhảy xuống khi sẵn sàng.
...
1. **Tự tải khi mở trang.** Boss OS quét các nguồn dữ liệu (POS, kênh, quảng cáo...) đã kết nối và điền số. Nếu chưa đấu nguồn kinh doanh nào, panel hiện thay bằng số Agents / Skills / Workflows của vault.
2. **Bấm nút ⟳** cạnh chữ SỐ LIỆU KINH DOANH để lấy lại số mới nhất.
3. **Trong lúc trò chuyện.** Khi bạn hỏi về tình hình kinh doanh, Boss OS có thể gắn kèm một khối số liệu ẩn trong câu trả lời (đánh dấu `BOSS_METRICS`). Boss OS tự gỡ khối này ra khỏi phần chữ đọc và đẩy con số lên các thẻ ở panel trái. Bạn chỉ thấy thẻ số liệu đổi, không thấy đoạn kỹ thuật đó.

Số liệu của phiên gần nhất được nhớ lại: mở trang lần sau vẫn thấy ngay, kèm ghi chú "phiên trước", rồi Boss OS làm mới ngầm. Chi tiết về nguồn số liệu xem [MCP & số liệu kinh doanh](09-mcp-va-so-lieu.md).

## Ý nghĩa dòng chữ trạng thái giữa màn hình

Dòng chữ ngay dưới quả cầu cho biết Boss OS đang làm gì:
...
| ĐANG NÓI | Boss OS đang đọc câu trả lời |
...
| **Esc** | Dừng Boss OS đang trả lời/đọc, tắt chế độ rảnh tay, thu nhỏ khung chat phóng to |

## Mẹo

- Muốn nói dài nhiều câu mà không sợ Boss OS gửi sớm, dùng chế độ rảnh tay (nút mic) và nói liền mạch; chỉ ngừng hẳn khi thật sự nói xong.
- Nghe Boss OS đọc lâu, muốn im lặng đọc chữ: tắt công tắc "Đọc trả lời bằng giọng", câu trả lời vẫn hiện đầy đủ dạng chữ.
- Đưa nhiều ảnh chụp màn hình cùng lúc bằng cách kéo - thả tất cả vào cửa sổ, Boss OS xử lý từng cái.
...
- **Trình duyệt không nghe được.** Boss OS báo "Trình duyệt không hỗ trợ giọng nói. Dùng Chrome/Edge." Hãy mở dashboard bằng Chrome hoặc Edge.
...
- **Không nghe thấy Boss OS đọc.** Kiểm tra nút loa ở góc trên phải có bị gạch chéo (tắt tiếng) không; kiểm tra âm lượng máy. Bấm "▶ Nghe thử" để kiểm tra riêng phần đọc.
