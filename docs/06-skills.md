...
Skill là "kỹ năng đóng gói" cho Boss OS: một hướng dẫn viết sẵn để AI làm đúng một loại việc theo chuẩn của bạn (ví dụ viết email bán hàng, dựng trang bán hàng, nghiên cứu chuyên sâu). Khi bạn nói một câu khớp với mô tả của skill, Boss OS tự lấy hướng dẫn đó ra dùng, không cần bạn dán lại quy trình mỗi lần.

Trang này hướng dẫn quản lý skill trong dashboard: xem theo nhóm, tìm kiếm, bật/tắt, thêm, sửa, xoá, và cách nhờ Boss OS tự tạo skill bằng lời.

## Tính năng này là gì

Một skill trong Boss OS là một thư mục chứa file `SKILL.md`, đặt tại `.claude/skills/<slug>/SKILL.md` bên trong brain đang chọn. File này có 3 phần đầu (frontmatter) quan trọng:

- `name`: tên hiển thị của skill.
- `description`: mô tả ngắn, chính là **trigger** - quyết định KHI NÀO skill được kích hoạt. Viết rõ trong đó "dùng khi người dùng muốn làm X, nhắc tới Y" thì Boss OS mới biết lúc nào nên tự bật.
...
Điểm cần nhớ: chỗ dashboard hiển thị chính là chỗ Claude Code thật sự nạp skill lúc chạy. Nghĩa là **những gì bạn thấy trong danh sách cũng chính là những gì Boss OS dùng được**, không có bản "ẩn" nào khác.

## Trigger hoạt động thế nào

Skill không phải nút bấm thủ công. Nó tự kích hoạt dựa trên trường `description`. Khi bạn gõ hoặc nói một yêu cầu, Boss OS (chạy trên engine Claude Code) so khớp yêu cầu với `description` của các skill đang bật, thấy khớp thì nạp hướng dẫn tương ứng.
...
Boss OS chia skill làm 2 loại:

- **Skill hệ thống** (thẻ có nhãn "hệ thống"): chức năng mặc định của Boss OS - hiện gồm `boss-builder` (dạy Boss OS tự tạo agent/skill/workflow/loop), `ingest-source`, `query-wiki`, `lint-wiki`. Bản gốc nằm trong thư mục cài đặt của app (không nằm trong brain), nên chúng **có mặt ở mọi brain** và **tự cập nhật khi bạn cập nhật Boss OS** lên phiên bản mới. Loại này không xoá được từ dashboard (lỡ xoá file thủ công thì lần khởi động sau tự cài lại); muốn ngừng dùng thì **tắt** như skill thường - trạng thái tắt được giữ nguyên qua mọi lần cập nhật.
...
Bạn vẫn **Sửa** được skill hệ thống. Khi đó bản trong brain trở thành bản riêng của bạn: Boss OS giữ đúng chỉnh sửa đó và ngừng tự cập nhật đè lên. Muốn quay về bản chuẩn (kèm tự cập nhật), xoá thư mục skill đó trong `.claude/skills` của brain (bằng trang Tệp tin) rồi khởi động lại - bản hệ thống mới nhất sẽ được cài lại sạch.

Cùng cơ chế này, loop **Tự cải tiến Boss OS** (trang Loop) cũng là năng lực hệ thống: phần nội dung nhiệm vụ được cập nhật theo phiên bản app, còn trạng thái bạn chỉnh (bật/tắt, chế độ, chu kỳ) luôn được giữ nguyên.
...
Gõ vào ô **Tìm skill…** ở đầu danh sách. Boss OS lọc ngay khi bạn gõ, so khớp từ khoá với cả tên, mô tả và slug của skill. Bộ lọc tìm kiếm chồng lên bộ lọc nhóm: nếu đang đứng ở một nhóm cụ thể, tìm kiếm chỉ chạy trong nhóm đó; muốn tìm toàn bộ thì bấm **Tất cả** trước.
...
Khi bạn tắt một skill, Boss OS chuyển thư mục skill đó vào một chỗ riêng tên là `.disabled` (đường dẫn thành `.claude/skills/.disabled/<slug>`). Đây là cách **tắt thật**: skill nằm trong `.disabled` sẽ không được engine nạp nữa, nên Boss OS không còn tự dùng nó. Khi bật lại, thư mục được chuyển ngược ra `.claude/skills/<slug>`.

Bật/tắt không xoá nội dung skill. Bạn có thể tắt tạm rồi bật lại bất cứ lúc nào mà không mất hướng dẫn đã viết.

Nếu có lỗi khi đổi trạng thái, Boss OS báo "Không đổi được trạng thái" kèm lý do.
...
2. Điền các ô trong biểu mẫu:
   ...
   - **Mô tả (description - quyết định khi nào skill kích hoạt)**: viết rõ trigger, nêu tình huống và từ khoá để Boss OS biết lúc nào nên bật skill này.
   - **Nội dung (SKILL.md - hướng dẫn cho AI)**: viết hướng dẫn chi tiết cho AI khi skill chạy (các bước, khung mẫu, quy tắc). Nếu để trống, Boss OS tự tạo nội dung tối thiểu từ tên và mô tả.
3. Bấm **💾 Lưu**. Muốn bỏ thì bấm **Huỷ**.

Khi lưu, Boss OS tự sinh **slug** từ tên skill: chuyển thành chữ thường, bỏ dấu tiếng Việt, thay khoảng trắng bằng gạch nối (ví dụ "Viết email" thành `viet-email`). Slug ASCII không dấu giúp Claude Code nạp skill ổn định hơn. Thư mục `.claude/skills/<slug>/SKILL.md` được tạo tự động, bạn không cần tự tạo file.
...
2. Boss OS hỏi xác nhận: `Xoá skill "<tên>"? Sẽ xoá cả thư mục .claude/skills/<slug>.`
3. Bấm đồng ý để xoá.
...
- **Tạo skill nhưng Boss OS không tự dùng:** kiểm tra `description` đã nêu rõ trigger chưa, và skill có đang **bật** không (thẻ không bị mờ, ô đánh dấu có tích). Ngoài ra skill chỉ chạy khi engine là Claude Code CLI, xem [Models & engine](10-models-va-engine.md).
...
- [Trò chuyện & giọng nói](02-tro-chuyen-va-giong-noi.md) - nhờ Boss OS tạo skill bằng lời.
