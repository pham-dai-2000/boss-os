...
Codex        profile toml 1 entry ---+--> [ BOSS MCP HUB ] --> Pancake POS (shop A) (shop B)
...
- Đặt tên tool: connector có 1 connection thì giữ tên gọn `pos__pos_order`; nhiều connection thì `pos_kim-khi-ha-loc__pos_order`. Kèm meta-tool `boss_connections` (liệt kê connector + tài khoản + quyền) để model tự biết đang có nguồn nào.
