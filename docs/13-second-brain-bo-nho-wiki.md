...
Second Brain là "bộ não ngoài" của Boss: một thư mục chứa các ghi chú Markdown mà Boss đọc, tích luỹ và nhớ lâu dài. Nhờ nó, Boss không chỉ trả lời câu hỏi trong lúc chat mà còn nhớ về anh, về việc kinh doanh của anh, và ngày càng hiểu anh hơn theo thời gian.

Trang này hướng dẫn: hiểu Second Brain gồm những gì, cách tạo và chọn nhiều "não" khác nhau, cách để Boss nhớ (bộ nhớ dài hạn), và cách "tiêu hoá" tài liệu (INGEST) để biến file thô thành tri thức dùng lại được.
...
| Memory | `memory/` | Bộ nhớ sống: những gì Boss nhớ về anh và doanh nghiệp. |
...
- **Wiki** là tri thức đã tinh lọc, nối với nhau thành mạng, chính là thứ Boss vẽ ra trong [Đồ thị tri thức 3D](03-do-thi-tri-thuc-3d.md).
- **Memory** là bộ nhớ dài hạn giúp Boss "nhớ anh".
...
## Đa-brain: nhiều bộ não trong một Boss
...
3. Toàn bộ Boss (chat, đồ thị, bộ nhớ, agents) lập tức chuyển sang brain đó. Không cần tải lại trang.

### Tạo brain mới

1. Bấm nút ➕ ở cạnh ô chọn brain.
2. Nhập **Tên brain mới** vào ô hiện ra, rồi xác nhận.
3. Boss tạo một thư mục con mới kèm sẵn cấu trúc chuẩn (sources, wiki, memory, agents, workflows, attachments) và chọn ngay brain vừa tạo.
...
3. Boss dùng thư mục đó làm brain. Lưu ý: folder ngoài chỉ được "trỏ tới", nút 🗑 sẽ không xoá nó khỏi ổ đĩa (chỉ bỏ khỏi danh sách).
...
4. Nếu gõ đúng, brain bị xoá và Boss quay về Brain Default. Gõ sai tên thì thao tác bị huỷ.
...
## Bộ nhớ dài hạn: làm Boss "nhớ anh"

Bộ nhớ sống nằm ở `memory/` trong brain đang chọn, gồm:

- `memory/MEMORY.md`: chỉ mục, mỗi ký ức một dòng. File này được nạp sẵn vào đầu mỗi câu hỏi, nên Boss luôn "nhớ nền" về anh.
...
Boss phân 4 loại ký ức: thông tin về anh (`user`), cách anh thích làm việc (`preference`), sự thật về kinh doanh (`business`), và quyết định đã chốt (`decision`).
...
Mô tả ngay dưới ghi rõ: "Ký ức lưu ngay trong vault đang chọn - đổi máy chỉ cần trỏ lại vault là Boss nhớ như cũ." Nghĩa là bộ nhớ đi theo thư mục, không kẹt trong máy nào.

### Ép Boss ghi nhớ một điều
...
Khi anh dùng các cụm đó, Boss bắt buộc tạo ngay một ký ức mới: viết một file trong `memory/facts/` và thêm một dòng vào `MEMORY.md`. Ví dụ: "Nhớ điều này: shop tôi nghỉ bán Chủ Nhật" sẽ được lưu thành một sự thật `business`.

Boss chỉ ghi những điều bền vững, đáng nhớ. Nó bỏ qua chuyện nhất thời và không nhân bản ký ức đã có (trùng thì cập nhật file cũ).

### Học từ hội thoại (vòng hợp nhất / rewire)

Đây là vòng lặp giúp Boss thông minh dần: đọc lại hội thoại gần đây, rút ra sự thật mới, gộp trùng lặp, bỏ ký ức đã sai, và **đúc kết khái niệm tái dùng được vào Wiki**.
...
2. Nút đổi thành "🧠 Đang học..." trong lúc Boss đọc lại hội thoại.
...
Ngay dưới đó có ô tích **Tự học sau mỗi 6 lượt**. Khi bật (mặc định), cứ sau 6 lượt hội thoại Boss tự chạy một vòng học nền, không làm gián đoạn anh. Bỏ tích nếu muốn tắt.

Điểm quan trọng của vòng này: Boss phân biệt rõ hai thứ. Sự thật về anh và doanh nghiệp vào `memory/facts/`. Còn khái niệm, framework, quy trình dùng lại được thì chưng cất thành trang Wiki (có `[[wikilink]]`). Cái nào ra cái nấy, nhờ vậy đồ thị tri thức dày lên chứ không lẫn lộn với ghi chú cá nhân.

## INGEST: tiêu hoá tài liệu thành tri thức

INGEST là quy trình biến một file thô (bài viết, ảnh chụp, ghi chú) thành nguồn trong `sources/`, rồi từ đó chưng cất lên Wiki. Kết quả: Boss tóm tắt, rút insight, viết Wiki và có thể gợi ý task.
...
3. Mặc định, Boss **chỉ đọc file rồi trả lời**, chưa lưu đi đâu. Nếu anh chỉ cần tóm tắt nhanh thì gõ câu hỏi bình thường.
4. Muốn Boss lưu và tiêu hoá, hãy nói rõ trong tin nhắn một trong các cụm: **"lưu vào source"**, **"ingest"**, hoặc **"ghi vào second brain"**.
5. Khi đó Boss sẽ:
...
6. Từ nguồn đó, Boss rút insight và cập nhật Wiki, đồng thời có thể đề xuất task nếu tài liệu mở ra việc cần làm.

### Ép Boss xử lý mẻ nguồn theo lịch

Nếu anh dồn nhiều nguồn chưa xử lý, có thể để Boss tự làm nền:
...
LINT tìm 8 loại vấn đề: mâu thuẫn giữa các trang, thông tin cũ chưa cập nhật (stale claim), trang không có ai trỏ tới (orphan page), khái niệm thiếu trang riêng (missing page), liên kết `[[...]]` gãy (broken wikilink), trang trùng lặp, vùng kiến thức mỏng (gap), và câu hỏi mở chưa được lấp (open-question). Đọc danh sách rồi tự quyết định sửa cái nào, đừng để Boss sửa hàng loạt cùng lúc.

## Mẹo

- **Tách brain theo mục đích.** Một brain cho kinh doanh, một brain cho cá nhân sẽ giúp đồ thị tri thức và bộ nhớ gọn, đỡ nhiễu.
- **Nói "nhớ điều này" cho những gì bền vững.** Ví dụ ngách sản phẩm, kênh bán chính, quyết định giá. Đừng ghi chuyện nhất thời (hôm nay bận, tin nhắn vừa gửi), Boss vốn đã bỏ qua loại này.
- **Muốn tiêu hoá tài liệu thì phải nói rõ.** Chỉ thả file không đủ để lưu, phải kèm cụm "lưu vào source" hoặc "ingest". Nếu chỉ hỏi bình thường, Boss đọc xong là thôi.
- **Bộ nhớ đi theo thư mục.** Đổi máy hoặc chuyển VPS, chỉ cần trỏ Boss về đúng thư mục brain là mọi ký ức và Wiki còn nguyên.
...
- **Thả file mà không thấy vào Sources.** Đúng như thiết kế: mặc định chỉ đọc. Phải gõ kèm "lưu vào source" / "ingest" trong tin nhắn thì Boss mới tạo file `.md` trong `sources/`.
...
- **Brain báo cấu trúc chưa chuẩn.** Vào Tổng quan, mục Cấu trúc brain, bấm **Chuẩn hóa brain đang chọn** để Boss gom lại các thư mục.
