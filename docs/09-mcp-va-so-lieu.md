...
Trang **Kết nối** là nơi bạn "đấu" Boss vào các công cụ bạn đang dùng: Pancake POS, Zalo, Webcake Landing, Botcake, quảng cáo Meta/Google/TikTok, lịch, CRM... Sau khi đấu, Boss đọc được số liệu THẬT và (nếu bạn cho quyền) thao tác thật trên các công cụ đó. Trang này hướng dẫn: kết nối một dịch vụ từ Kho, nối nhiều tài khoản, phân quyền, xem nhật ký, và cách đọc số liệu.

## Tính năng này là gì

Bên dưới, mỗi kết nối là một "đường ống" MCP (Model Context Protocol) nối Boss tới dịch vụ ngoài - nhưng bạn không cần biết chi tiết đó. Điểm mới từ bản 0.9:

- **Kho kết nối cài sẵn**: chọn dịch vụ, dán API key (hoặc quét QR với Zalo) là xong. Boss tự kiểm tra key và tự đặt tên tài khoản (ví dụ lấy đúng tên cửa hàng từ Pancake POS). Không còn phải gõ URL hay header.
...
- **Mọi bộ não dùng chung**: Claude Code, ChatGPT (Codex), OpenRouter, OpenAI API, Anthropic API đều dùng chung kho Kết nối này qua "hub" của Boss - đấu một lần, đổi model thoải mái.
- **Phân quyền cứng**: mỗi tài khoản có mức quyền. Boss CHẶN thật sự (không phải chỉ nhắc bằng lời) các thao tác vượt quyền, ví dụ tạo đơn khi đang ở mức Chỉ đọc.
...
3. Bấm **Kết nối**. Boss tự kiểm tra key - đúng thì hiện "✓ Đã kết nối: <tên cửa hàng>" và tài khoản xuất hiện ở khu Đã kết nối. Key sai thì báo lỗi ngay tại chỗ.
4. Có nhiều cửa hàng? Bấm **+ Thêm tài khoản** trên thẻ Pancake POS, dán key của shop tiếp theo. Mỗi shop một chip riêng.

Pancake POS mặc định ở mức **Chỉ đọc** - Boss xem được doanh thu, đơn, khách... nhưng không thể tạo đơn hay đụng tiền. Muốn Boss thao tác thật, xem mục Phân quyền bên dưới.

### 2. Kết nối Zalo (quét QR)

Cần Node.js 20+ trên máy chạy Boss (tải tại nodejs.org, cài một lần).
...
3. Mở Zalo trên điện thoại > biểu tượng QR góc trên > quét mã trong màn hình Boss.
4. Quét xong, tài khoản tự xuất hiện. Nối thêm số Zalo khác bằng **+ Thêm tài khoản** - các tài khoản chạy cô lập, không giẫm nhau.

Zalo mặc định được cả đọc lẫn gửi tin (Toàn quyền) theo lựa chọn của bạn khi kết nối - Boss chỉ gửi tin khi bạn yêu cầu trực tiếp trong chat, và loop chạy nền KHÔNG bao giờ được tự gửi. Có giới hạn tần suất tự động để tránh spam gây khoá tài khoản.
...
- **Systeme.io** (MCP chính chủ, dán key là xong): vào systeme.io > Cài đặt hồ sơ > "MCP & API keys" > tạo MCP key (hạn tối đa 90 ngày), dán vào. Boss quản lý được liên hệ, tag, newsletter, phễu. Mặc định Chỉ đọc.
- **Lark** (MCP chính chủ, chạy local, cần Node.js 18+): nhắn tin, tài liệu, bảng dữ liệu Base, wiki, danh bạ trong Lark. Tạo một Lark app tại open.larksuite.com/app, cấp quyền (im, docx, bitable, contact...), lấy App ID + App Secret dán vào. Boss chỉ làm được đúng phạm vi quyền bạn cấp cho app. Mặc định Chỉ đọc - gửi tin nhắn và cấp quyền file phải nâng Toàn quyền.

### 3. Kết nối Webcake Landing / Botcake

- **Webcake Landing**: lấy JWT tại webcake.io > Cài đặt > Mã truy cập > Tạo API keys, dán vào. Boss sẽ thiết kế/sửa landing page bằng lời nói. Cần Node.js 18+.
- **Botcake**: mở Botcake > Cấu hình > Tích hợp > Public API > Tạo API Key; dán Page ID + key. Boss đọc được khách hàng, tag, flow và (nếu cho Toàn quyền) gửi flow tới khách.
...
- **Google Calendar** và **Gmail** (2 kết nối riêng, MCP chính chủ của Google, chạy remote nên dùng được cả trên VPS): Calendar xem lịch, tìm chỗ trống, tạo/sửa/xoá sự kiện, nhắc hẹn; Gmail đọc/tìm thư, soạn NHÁP, gắn nhãn. Điểm an toàn: server Gmail chính chủ KHÔNG có tool gửi thẳng, nên Boss luôn dừng ở bản nháp để bạn tự bấm gửi. Cần tạo OAuth client một lần (console.cloud.google.com > Thông tin xác thực > OAuth client ID loại "Ứng dụng web", thêm URI chuyển hướng đúng như cửa sổ kết nối chỉ, và thêm email mình vào Test users). Dán Client ID + Secret, bấm Kết nối là trình duyệt mở cho bạn đăng nhập Google. Dùng CHUNG một OAuth client cho cả Calendar lẫn Gmail (chỉ cần bật thêm API tương ứng). Cả hai mặc định Chỉ đọc; nâng lên Ghi nháp để tạo sự kiện/soạn nháp, Toàn quyền mới xoá được sự kiện.
- **Google Workspace** (Gmail + Lịch + Drive + Docs trong 1 kết nối, chạy local): cần tạo OAuth client trong Google Cloud một lần (~10 phút, hướng dẫn từng bước trong cửa sổ kết nối); lần đầu dùng, trình duyệt mở để bạn bấm đồng ý. Mặc định ở mức Ghi nháp: Boss soạn nháp mail, tạo lịch, tạo tài liệu được nhưng KHÔNG tự gửi mail hay xoá gì - bật Toàn quyền phải xác nhận rủi ro. Chọn cái này nếu muốn CẢ Drive/Docs/Sheets trong một mối; nếu chỉ cần Lịch + Gmail thì 2 kết nối riêng ở trên gọn hơn (ít công cụ, chạy remote).

Mẹo: nếu chỉ cần Gmail/Lịch/Drive và bạn dùng engine Claude Code, cách nhanh hơn là bấm Connect ngay trong app Claude (claude.ai > Settings > Connectors) - Boss tự thấy chúng ở khu "MCP từ Claude Code".

### 5. Kết nối quảng cáo (Meta Ads, Google Ads, TikTok Ads)

Cả 3 mặc định ở mức **Chỉ đọc** - Boss xem báo cáo, phân tích chi phí/hiệu quả nhưng không đụng được vào chiến dịch.

- **Meta Ads (Facebook & Instagram)**: dễ nhất - bấm **Kết nối**, đăng nhập Facebook, chọn tài khoản quảng cáo và mức quyền (chọn "chỉ xem" nếu chỉ cần báo cáo). Không cần tạo app hay dán key; đây là MCP chính chủ của Meta (đang beta, mở dần theo từng tài khoản - bị từ chối nghĩa là tài khoản chưa được mở). Nếu sau này bạn bật Toàn quyền để Boss tạo/sửa chiến dịch: mọi chiến dịch tạo qua đây luôn ở trạng thái TẠM DỪNG, bạn phải tự bật trong Ads Manager - tiền chỉ chạy khi chính bạn bấm.
...
- **TikTok Ads**: TikTok chưa mở MCP chính chủ (mới công bố tại TikTok World 5/2026), nên Boss dùng server cộng đồng chạy trên Marketing API chính thức - thuần chỉ đọc (tài khoản, chiến dịch, báo cáo). Tạo app Marketing API tại business-api.tiktok.com, lấy App ID + Secret + Access Token dán vào. Khi TikTok mở bản chính chủ sẽ thay trong kho.

Google Ads và TikTok Ads chạy local qua công cụ `uv` - máy chạy Boss cần cài một lần: `winget install astral-sh.uv` (Windows) hoặc xem docs.astral.sh/uv.
...
- **Đặt làm mặc định**: khi có nhiều tài khoản cùng dịch vụ, Boss ưu tiên tài khoản mặc định khi bạn không nói rõ shop nào.
- **Đổi tên** / **Tắt tạm** / **Xoá**.
- **Đổi quyền**: xem mục Phân quyền.
- **Chặn tool cụ thể**: dành cho người rành - gõ tên tool muốn cấm hẳn.
- **Nhật ký gọi tool**: xem Boss đã gọi gì, lúc nào, bị chặn gì.

### 7. Phân quyền 3 mức (quan trọng)

Mỗi tài khoản có một mức quyền, Boss chặn CỨNG tại chỗ:
...
Thông minh hơn bản cũ: Boss hiểu cả công cụ "2 trong 1" của Pancake - cùng tool đơn hàng, hỏi `danh sách đơn` thì cho qua, `tạo đơn` thì chặn nếu chưa đủ quyền.
...
Dịch vụ chưa có trong Kho? Bấm thẻ **Tự thêm (nâng cao)** - form kỹ thuật như bản cũ (URL/lệnh + header/env, hỗ trợ HTTP, SSE, stdio). Dịch vụ đăng nhập kiểu OAuth chuẩn MCP thì Boss tự mở trang đăng nhập và tự giữ token, chạy được cả trên VPS.

### 9. Chế độ "Chỉ dùng kết nối của Boss" (strict)

Tick ô này ở khu Đã kết nối nếu muốn Boss CHỈ dùng các kết nối khai ở đây, bỏ qua MCP cài sẵn trong Claude Code trên máy - kiểm soát chặt, tránh gọi nhầm công cụ của tài khoản Claude.

## Đọc số liệu

Không đổi so với trước: hỏi trực tiếp trong chat ("hôm nay bán được bao nhiêu, so hôm qua thế nào?"), Boss gọi đúng nguồn, trả lời theo công thức số liệu + so kỳ trước + nguyên nhân + đề xuất, và tự đẩy 3-6 chỉ số lên bảng số liệu cột trái trang Boss. Kỳ đã đóng lưu cache trong `05 - Data Cache/` của brain. Có nhiều shop thì nói rõ tên shop, không nói thì Boss dùng tài khoản mặc định.

## Mẹo

- Đặt tên tài khoản theo cửa hàng cho dễ gọi trong chat ("shop Kim Khí" vs "shop 2").
- POS cứ để Chỉ đọc nếu bạn chỉ cần xem báo cáo - không lo Boss vô tình tạo đơn.
- Tin nhắn đầu sau khi bật máy có thể hơi chậm với kết nối dạng chạy local (Zalo, Webcake) do phải khởi động công cụ - các lượt sau nhanh vì Boss giữ kết nối sống.
- Nhật ký gọi tool là chỗ đầu tiên nên xem khi nghi Boss "làm gì đó lạ".
...
- **Tool bị chặn kèm dòng "đang ở mức quyền hạn chế"**: đúng thiết kế - nâng quyền tài khoản trong menu chip nếu bạn thật sự muốn Boss làm việc đó.
