#!/usr/bin/env bash
# ============================================================================
# Boss OS - Linux/macOS native installer (no Docker)
#   ./install.sh
...
if need_node; then
  ...
  if need_node; then
    ...
    mkdir -p "$HOME/.boss"; rm -rf "$HOME/.boss/node"
    tar xf "$tmp/n.tar.xz" -C "$tmp"; mv "$tmp"/node-v22* "$HOME/.boss/node"; rm -rf "$tmp"
    mkdir -p "$HOME/.local/bin"
    ln -sf "$HOME/.boss/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$HOME/.boss/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$HOME/.boss/node/bin/npx"  "$HOME/.local/bin/npx"
...
grep -q '^BOSS_HOST=' .env || echo "BOSS_HOST=127.0.0.1" >> .env
...
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  ...
  $SUDO tee /etc/systemd/system/boss.service >/dev/null <<UNIT
[Unit]
Description=Boss OS
...
Environment="BOSS_HOST=127.0.0.1"
Environment="BOSS_PORT=7777"
Environment="BOSS_STATE_DIR=$APP_DIR/server"
...
UNIT
  ...
  $SUDO systemctl enable --now boss.service
  ok "Service installed. Logs: journalctl -u boss -f"
else
  warn "systemd not available - starting under nohup..."
  ( cd "$APP_DIR/server" && BOSS_STATE_DIR="$APP_DIR/server" nohup "$PY" -m uvicorn main:app --host 127.0.0.1 --port 7777 > "$APP_DIR/server/boss.log" 2>&1 & )
  ok "Started. Logs: $APP_DIR/server/boss.log"
fi

echo ""
ok "Boss OS is up at: http://127.0.0.1:7777"
