...
def _empty_mcp_file() -> Optional[str]:
    try:
        p = Path(_tempfile.gettempdir()) / "boss-empty-mcp.json"
...
def mcp_open_auth_terminal():
    ...
    try:
        if os.name == "nt":
            subprocess.Popen('start "Boss - Xac thuc MCP (go /mcp)" cmd /k claude', shell=True)
        else:
...
class ClaudeCLI:
    ...
    async def query(self, prompt: str) -> AsyncIterator[dict]:
        ...
        def reader_thread():
            ...
            trình treo làm đói tài nguyên → treo server. Chỉnh bằng BOSS_CLAUDE_IDLE_TIMEOUT."""
            proc = None
            tinfo = {"timed_out": False}
            last = {"t": time.time()}            # cập nhật mỗi dòng stdout → "còn sống"
            IDLE = float(os.getenv("BOSS_CLAUDE_IDLE_TIMEOUT", "180"))
            try:
                    ...
                    while p.poll() is None:
                        if time.time() - last["t"] > IDLE:
                            ...
                            asyncio.run_coroutine_threadsafe(queue.put({"__error__":
                                ...
                                f"(tăng BOSS_CLAUDE_IDLE_TIMEOUT nếu tác vụ thật sự dài)"}), loop)
                            return
