"""
Cấu hình tập trung của Boss OS - đọc/ghi server/settings.json (gitignored, chứa secret).
...
def require_login():
    ...
    v = os.getenv("BOSS_REQUIRE_LOGIN", "").strip().lower()
    if v in ("1", "true", "yes", "on"):
        return True
    if v in ("0", "false", "no", "off"):
        return False
    # FAIL-CLOSED: bind KHÔNG phải loopback (0.0.0.0, ::, IP LAN…) → coi là public → bắt buộc login.
    # Chỉ tắt khi nghe thuần localhost. (Localhost + tunnel: đặt BOSS_REQUIRE_LOGIN=1.)
    host = os.getenv("BOSS_HOST", "127.0.0.1").strip().lower()
