"""
learn.py - Engine TỰ HỌC của Boss OS (rewire sau lượt + auto-Wiki + skill + curator).
...
ALLOWED_WRITE_PREFIXES = ["memory", "Memory", "Wiki", ".claude/skills", "Boss"]
...
class LearnFeature:
    ...
    def _build_digest(self, brain: str, convs: List[str]) -> str:
        ...
        for sid in (convs or [])[:3]:
            ...
            for m in tail:
                who = "NGƯỜI DÙNG" if m["role"] == "user" else "BOSS(assistant)"
    ...
    def _build_prompt(self, caps: dict, brain: str, digest: str) -> str:
        ...
        return (
            "BẠN LÀ VÒNG HỌC READ-ONLY của Boss OS. TUYỆT ĐỐI KHÔNG ghi/sửa/xoá file, KHÔNG gọi tool ghi. "
            ...
            "• skill = quy trình nhiều bước Boss OS VỪA TỰ LÀM, có công thức lặp lại.\n"
            "• task = VIỆC NỀN cụ thể đáng giao Boss OS tự làm sau (yêu cầu lặp lại / việc bỏ dở / câu hỏi mở).\n\n"
            "PROVENANCE (bắt buộc, chống bịa): 'user'=user khẳng định; 'source'=trích nguồn có tên; "
            "'assistant'=CHÍNH BOSS tự nói không nguồn. ⚠ Mục wiki provenance='assistant' sẽ BỊ LOẠI "
    ...
    async def _verify_skills(self, brain: str, skills: List[dict], cfg: dict) -> List[dict]:
        ...
        prompt = ("Một vòng học đề xuất tạo các SKILL sau (Boss OS tự quan sát từ việc đã làm). "
            ...
            if caps.get("wiki"):
                ...
                for w in (manifest.get("wiki") or []):
                    ...
                    if (w.get("provenance") == "assistant"):
                        self._append_open_question(brain, wiki_dir, title, "provenance=assistant (Boss OS tự nói, cần xác minh)", written_paths, root)
    ...
    def _log(self, brain: str, kind: str, title: str, body: str) -> None:
        try:
            d = Path(self.deps.brain_root(brain)) / "Boss" / "learn-log"
            d.mkdir(parents=True, exist_ok=True)
            now = _now_vn()
    ...
    def _make_router(self) -> APIRouter:
        ...
        async def learn_review(brain: str = Query("brain"), limit: int = Query(20)):
            """Danh sách commit học gần nhất (cho panel 'Boss đã tự học gì') + trạng thái git."""
            root = self.deps.brain_root(brain)
