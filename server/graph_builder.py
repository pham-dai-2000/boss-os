...
def _nfc(s: str) -> str:
    """Chuẩn hoá Unicode về NFC (gộp dấu). Zip tạo trên macOS lưu tên file dạng NFD (tách dấu)
    → tên hiển thị vỡ font (ký tự ╠) + [[wikilink]] (NFC) so không khớp tên file (NFD) → mất cạnh nối."""
...
def build_graph(roots: List[str], max_files: int = 2000) -> Dict:
    ...
    for node_id, fpath in all_files:
        ...
        for match in WIKILINK_RE.finditer(content):
            ...
            target_stem = _nfc(target_raw.split("/")[-1].strip()).lower()
    ...
    for n in node_list:
        ...
        cat = _nfc(re.sub(r"^\d+\s*[-_.]\s*", "", cat).strip())  # bỏ tiền tố "07 - " + gộp dấu NFC
        if cat:
            cat_counter[cat] += 1
