#!/usr/bin/env bash
# ============================================================================
# Boss OS - cập nhật lên bản mới nhất từ GitHub.
...
is_docker() {
  ...
  docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx boss
}
...
if [ "$MODE" = "docker" ] || { [ "$MODE" = "auto" ] && is_docker; }; then
  ...
  HTTPS_ARGS=""
  if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx boss-caddy; then
  ...
  if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files 2>/dev/null | grep -q '^boss\.service'; then
