// ============================================
// BOSS OS - Voice Layer (Web Speech API)
// ============================================
...
class BossVoice {
  constructor(opts = {}) {
    ...
    this.ttsEnabled = (localStorage.getItem("boss.ttsEnabled") !== "0");
    ...
    this.audioCtx = null;
    this.outAnalyser = null;   // âm Boss OS đọc (TTS)
  ...
  _pumpQueue() {
    if (!this.ttsEnabled) { this.speechQueue = []; this.isPlaying = false; return; }
  ...
  async _speakBackend(text) {
    if (!this.ttsEnabled) { this.isPlaying = false; return; }
  ...
  _playChunk(i) {
    if (!this.ttsEnabled) { this.isPlaying = false; return; }   // đã tắt tiếng → ngừng hẳn
    ...
    // CHỐNG CHỒNG TIẾNG: dừng mọi nguồn đang kêu (giọng browser + audio trước) trước khi phát chunk mới.
    try { this.synth.cancel(); } catch (e) {}
    if (this.currentAudio) { try { this.currentAudio.onended = null; this.currentAudio.onerror = null; this.currentAudio.pause(); } catch (e) {} }
    ...
    // Lỗi đoạn này → đọc bằng browser rồi đọc tiếp. CHỈ CHẠY 1 LẦN (onerror + catch có thể cùng bắn → tránh đọc đôi).
    const fallback = () => {
      if (audio.__fbUsed) return; audio.__fbUsed = true;
      if (this.ttsEnabled && this.ttsChunks) this._speakBrowser(this.ttsChunks[i], () => this._playChunk(i + 1));
    };
  ...
  _speakBrowser(text, onDone) {
    if (!this.ttsEnabled) { this.isPlaying = false; return; }   // đã tắt tiếng → không đọc bằng browser
    // CHỐNG CHỒNG TIẾNG: dừng audio Edge (nếu đang kêu) trước khi đọc bằng browser.
  ...
  stopSpeaking() {
    this.synth.cancel();
    // Xoá hàng đợi + chunk TRƯỚC khi pause → mọi callback (onended/catch) bung ra đều thấy rỗng, không đọc tiếp.
    this.ttsChunks = null;
    this.ttsQueue = [];
    this.speechQueue = [];
    this.isPlaying = false;
    if (this.currentAudio) {
      try { this.currentAudio.onended = null; this.currentAudio.onerror = null; this.currentAudio.pause(); this.currentAudio.src = ""; } catch (e) {}
      this.currentAudio = null;
    }
    if (this._preloaded && this._preloaded.audio) {
      try { this._preloaded.audio.onended = null; this._preloaded.audio.onerror = null; this._preloaded.audio.pause(); this._preloaded.audio.src = ""; } catch (e) {}
    }
    this._preloaded = null;
  }
  ...
  toggleTTS() {
    ...
    try { localStorage.setItem("boss.ttsEnabled", this.ttsEnabled ? "1" : "0"); } catch (e) {}
