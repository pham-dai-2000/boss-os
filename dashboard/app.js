...
const SESSION_KEY = "boss.session.v1";
let convo = [];            // [{role:"user"|"boss", text, atts}]
...
fetch("/config").then(r => r.json()).then(cfg => {
  document.getElementById("workspaceName").textContent = cfg.workspace_name || "Boss OS";
}).catch(() => {});
...
function setOrbState(state, label) {
  ...
  if (bossGraph) bossGraph.setThinking(thinking);
}

// ============================================
// Voice
// ============================================
const voice = new BossVoice({
  lang: "vi-VN",
  onStart: () => {
...
function handleMessage(data) {
  ...
  if (data.type === "status") {
    ...
    let bubble;
    if (!streamingBubble) { appendBossMessage(shownText); bubble = chatArea.lastChild; }
    ...
    if (finalText.trim()) recordTurn("boss", finalText);   // KHÔNG lưu lượt rỗng (tránh khôi phục bong bóng trống)
    maybeAutoLearn();
...
function sendMessage(text) {
  ...
  // Soạn message gửi Boss OS (kèm đường dẫn file trong Sources)
  let outMsg = msg;
  if (atts.length) {
  ...
  updateStopBtn();
  setOrbState("thinking", "ĐANG SUY NGHĨ");
  showActivity("Boss OS đang suy nghĩ...");   // hiện NGAY trong khung chat, không đợi server báo
...
// ============================================
// Phiên hội thoại lưu DB (panel Lịch sử - sessions-ui.js gọi qua window.BossSessions)
// ============================================
async function openStoredSession(id) {
  try {
    ...
    (sess.messages || []).forEach(m => {
      ...
      else if (m.role === "assistant") { appendBossMessage(m.content || ""); convo.push({ role: "boss", text: m.content || "", atts: [] }); }
    });
...
window.BossSessions = { open: openStoredSession, new: newChat, brain: () => currentBrainPath(), current: () => savedSessionId };
// Báo các UI khác (sidebar Lịch sử trong chat workspace) biết phiên/danh sách vừa đổi
function notifySessions() { try { window.dispatchEvent(new Event("boss:sessions-changed")); } catch (e) {} }
...
async function initGraph() {
  ...
  if (!window.BossGraph3D || !window.ForceGraph3D) {
    graphStats.textContent = "⚠ Lỗi tải thư viện 3D (kiểm tra mạng)";
    return;
  }
  bossGraph = new BossGraph3D(graph3dContainer, graphTooltip);
  await reloadGraph();
}

// Click node trong graph → Boss OS mở & thao tác note đó trong vault
...
async function reloadGraph() {
  ...
  const query = val.startsWith("path:")
    ...
    : `source=${val}`;
  try {
    const data = await bossGraph.load(query);
...
graphSource.addEventListener("change", () => {
  localStorage.setItem("boss.graphSource", graphSource.value);
  reloadGraph();
...
function connectGraphWatch() {
  ...
  graphWs.onmessage = (e) => {
    ...
    if (m.type !== "graph_add" || !bossGraph) return;
    const r = bossGraph.addOrUpdate(m.node, m.linkTargets, m.isNew);
    if (r && r.created) {
      const s = bossGraph.nodeStats();
  ...
  try {
    ...
    if (d.ok && d.missing === 0) {
      ...
      vbText.textContent = d.ok
        ...
        : `Cấu trúc vault chưa chuẩn cho Boss OS - thiếu: ${miss}.`;
...
function renderConceptLabels(categories, total) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    div.style.left = x + "%";
    div.style.top = y + "%";
    div.innerHTML = `<div class="cl-name">${escapeHtml((c.name || "").normalize("NFC").toUpperCase())}</div>` +
      `<div class="cl-meta">${c.count} note · <span class="cl-fire">${share}% Vault</span></div>`;
    container.appendChild(div);
    setTimeout(() => div.classList.add("show"), 120 + i * 110);
  }
  // Fix cứng: đẩy nhãn nào tràn mép trái/phải vào trong, luôn chừa khoảng đệm với lề.
  const clampEdges = () => {
    const host = container.getBoundingClientRect();
    const pad = 12;
    [...container.children].forEach(div => {
      div.style.marginLeft = "";                 // reset trước khi đo lại
      const r = div.getBoundingClientRect();
      const overLeft = (host.left + pad) - r.left;
      const overRight = r.right - (host.right - pad);
      if (overLeft > 0) div.style.marginLeft = Math.ceil(overLeft) + "px";
      else if (overRight > 0) div.style.marginLeft = -Math.ceil(overRight) + "px";
    });
  };
  requestAnimationFrame(clampEdges);
  // Chạy lại khi cửa sổ đổi kích thước (host rộng/hẹp khác đi)
  clearTimeout(renderConceptLabels._rz);
  window.removeEventListener("resize", renderConceptLabels._onRz || (() => {}));
  renderConceptLabels._onRz = () => { renderConceptLabels._rz = setTimeout(clampEdges, 150); };
  window.addEventListener("resize", renderConceptLabels._onRz);
}

// Brain folder tùy chọn - lưu localStorage, hiện trong dropdown
function loadCustomBrains() {
  const brains = JSON.parse(localStorage.getItem("boss.brains") || "[]");
  // Xóa option cũ
...
function addCustomBrain(path) {
  ...
  localStorage.setItem("boss.brains", JSON.stringify(brains));
  loadCustomBrains();
}
loadCustomBrains();
// Khôi phục folder đã chọn lần trước (mặc định: brain)
(function restoreGraphSource() {
  const saved = localStorage.getItem("boss.graphSource");
...
window.addEventListener("resize", () => { if (bossGraph) bossGraph.resize(); });

let _stopBtnTick = 0;
function pumpAudioLevel() {
  if (bossGraph) bossGraph.setLevel(voice.getLevel());
  // Cập nhật hiển thị nút stop ~6 lần/giây (theo dõi cả lúc Boss OS đang đọc)
  ...
  try {
    ...
    if (cards.length === 0) {
      ...
      if (fb.length) {
        ...
        metricCards.innerHTML = `<div class="metric-empty">${escapeHtml(note)}<br><span class="me-hint">Đấu thêm MCP (POS, kênh, quảng cáo...) để Boss OS báo cáo.</span></div>`;
        status.textContent = "";
      }
      return;
    }
...
async function agenticFallbackCards() {
  ...
  try {
    ...
    return [
      { label: "Agents", value: String((a.agents || []).length), sub: "trong Boss OS", trend: "flat" },
...
let autoLearn = localStorage.getItem("boss.autoLearn") !== "off";
if (autoLearnToggle) {
  autoLearnToggle.checked = autoLearn;
  autoLearnToggle.addEventListener("change", () => {
    autoLearn = autoLearnToggle.checked;
    localStorage.setItem("boss.autoLearn", autoLearn ? "on" : "off");
  });
}
...
async function doReflect(auto) {
  ...
  if (memResult) memResult.textContent = auto ? "🧠 Đang tự học nền..." : "Boss OS đang đọc lại hội thoại và rút ra ký ức...";
  try {
    const fd = new FormData();
    fd.append("brain", currentBrainPath());
...
async function uploadFile(file) {
  ...
  renderChips();
  try {
    // Chỉ STAGE để Boss OS đọc - KHÔNG tự convert/lưu. Lưu Sources chỉ khi user yêu cầu.
    const fd = new FormData();
...
const savedVoice = localStorage.getItem("boss.voice") || "vi-VN-HoaiMyNeural";
const savedRate = parseFloat(localStorage.getItem("boss.rate") || "1.10");
...
document.querySelectorAll('input[name="voice"]').forEach(r => r.addEventListener("change", () => { voice.setVoice(r.value); localStorage.setItem("boss.voice", r.value); }));
const savedRecLang = localStorage.getItem("boss.recLang") || "vi-VN";
...
document.querySelectorAll('input[name="recognitionLang"]').forEach(r => r.addEventListener("change", () => { voice.setRecognitionLang(r.value); localStorage.setItem("boss.recLang", r.value); }));
rateSlider.addEventListener("input", () => { const r = parseFloat(rateSlider.value); rateLabel.textContent = r.toFixed(2) + "×"; voice.setRate(rateToPct(r)); localStorage.setItem("boss.rate", r.toString()); });
...
function renderLoopStatus(c) {
  ...
  if (!c.enabled) { loopStatus.textContent = "Tắt - bật để Boss OS tự chạy nền"; return; }
...
if (loopEnabled) {
  ...
  lintBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      appendBossMessage(d.ok ? ("🩺 **LINT Wiki**\n\n" + d.report) : ("⚠ " + (d.error || "lỗi LINT")));
    } catch (e) { appendBossMessage("⚠ LINT lỗi mạng"); }
...
async function initAuthGate() {
  ...
  if (s.needs_setup) {
    ...
    if (_wizardMandatory) {
      ...
      const note = document.getElementById("wzErr"); if (note) note.textContent = "Đặt tài khoản + mật khẩu (≥8 ký tự) + MÃ THIẾT LẬP để bảo vệ Boss OS trên server công khai.";
    }
...
if (document.getElementById("settingsBtn")) {
  ...
  document.getElementById("saveGeneral").addEventListener("click", (e) => {
    _saveSetting("general", { workspace_name: document.getElementById("setWsName").value.trim() }, e.target)
      .then(() => { document.getElementById("workspaceName").textContent = document.getElementById("setWsName").value.trim() || "Boss OS"; });
  });
...
if (document.getElementById("wzFinish")) {
  document.getElementById("wzFinish").addEventListener("click", async () => {
    ...
    if (_wizardMandatory && !pass) { err.textContent = "Bắt buộc đặt mật khẩu khi chạy trên server công khai."; btn.disabled = false; btn.textContent = "Bắt đầu dùng Boss OS →"; return; }
    try {
      if (pass) {
        ...
        if (!d.ok) { err.textContent = d.error || "Đặt mật khẩu lỗi"; btn.disabled = false; btn.textContent = "Bắt đầu dùng Boss OS →"; return; }
      }
      ...
      location.reload();
    } catch (e) { err.textContent = "Lỗi mạng"; btn.disabled = false; btn.textContent = "Bắt đầu dùng Boss OS →"; }
  });
}
